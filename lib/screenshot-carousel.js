...
(function() {
  ...
  function initCarousel() {
    ...
    function setCarouselWidth(image) {
      ...
      // Calculate image width based on natural dimensions and max-height constraint
      const maxHeight = 320; // 20rem = 320px
      const naturalWidth = image.naturalWidth;
      const naturalHeight = image.naturalHeight;
      const maxWidth = carousel.parentElement.offsetWidth;
      
      // Calculate width if image is constrained by max-height
      let imageWidth;
      if (naturalHeight > maxHeight) {
        // Image is height-constrained, calculate proportional width
        const scale = maxHeight / naturalHeight;
        imageWidth = naturalWidth * scale;
      } else {
        // Image uses natural width
        imageWidth = naturalWidth;
      }
      
      // Set container width to image width, but don't exceed parent width
      // Add small buffer for shadows (box-shadow can extend beyond image)
      const targetWidth = Math.min(imageWidth + 40, maxWidth);
